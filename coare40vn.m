function A=coare40vn(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,cp,sigH)
% function [A,B]=coare40vn(uwind,vwind,zu,t,zt,qv,zq,P,ts,Rs,Rl,lat)
%
...
%
% A=coare35vn(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,NaN,NaN)  :  the wind-speed dependent formulation is used
% A=coare35vn(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,cp,NaN)   :  the wave-age dependent formulation is used
% A=coare35vn(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,cp,sigH)  :  the sea-state dependent formulation is used
%
% The sea-state dependent formulation requires both cp and sigH.
%
%********************************************************************
% The cool skin option is retained but the warm layer code is removed.  
% An m-file that applies the warm layer correction and calls this version 
% of COARE 3 (coare35vnWarm) is available.
%
...
% 
%A=[usr tau hsb hlb hbb hsbb wbar  tsr qsr zot zoq Cd Ch Ce  L zet dter dqer tkt Urf Trf Qrf RHrf UrfN Rnl Le rhoa UN U10 U10N Cdn_10 Chn_10 Cen_10 RF Qs Evap T10 Q10 RH10];
...
if waveage && seastate
    disp('You are about to use the seastate dependent parameterization.')
%     display('Press any key to continue ...') % Siqi
%     pause
end
if waveage && ~seastate
    disp('You are about to use the waveage dependent parameterization.')
%     display('Press any key to continue ...') % Siqi
%     pause
end
...
Q  = qsat26air(t,P,rh);  % specific humidity of air (g/kg)
% [Q,Pv]  = qsat26air(t,P,rh);  % specific humidity of air (g/kg)   % Siqi
Q=Q./1000;
...
cpa  = 1004.67;
% cpv  = cpa*(1+0.84*Q);   % Siqi
rhoa = P*100./(Rgas*(t+tdk).*(1+0.61*Q));
% rhodry = (P-Pv)*100./(Rgas*(t+tdk));  % Siqi
...
charnC=a1*u10+a2;  %wind-speed dependent coefficients
k=find(u10>umax);
charnC(k)=a1*umax+a2;
...
for i=1:9%nits
    ...
    zet=von.*grav.*zu./tv.*(tsr.*(1+0.61*Q) +.61*ta.*qsr)./(usr.^2);
%     zet=von.*grav.*zu./tv.*(tsr +.61*ta.*qsr)./(usr.^2);
    if waveage
        if seastate
            charn=charnS;
        else
    ...
    rr=zo.*usr./visa;
    zoq=min(1.6e-4,6e-3./rr.^1.6);   % These thermal roughness lengths give Stanton   
    zot=zoq;                         % and Dalton numbers for COARE 4.0
