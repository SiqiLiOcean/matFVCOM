...
%
%==========================================================================
function [h, para] = w_2d_vector(wgrid, u, v, varargin)

varargin = read_varargin(varargin, {'Stride'},  {1});
...
nx = wgrid.nx;
ny = wgrid.ny;
x = wgrid.x(1:Stride:nx, 1:Stride:ny); x = x(:);
y = wgrid.y(1:Stride:nx, 1:Stride:ny); y = y(:);
u = u(1:Stride:nx, 1:Stride:ny);       u = u(:);
v = v(1:Stride:nx, 1:Stride:ny);       v = v(:);

% Rotate the current if needed
if wgrid.rotate~=0
    [u, v] = rotate_vector(u, v, wgrid.rotate);
end
