...
%
%==========================================================================
function tidestruc = create_tidestruc(name, amplitude, phase)
...
frequency0 = [
  ...
  for i = 1 : length(name)
      k = find(strcmp(cellstr(name0),name{i}));
      if isempty(k)
          error(['Unfound tide constituent name: ' name{i}])
      end
      
      %       tidestruc.name(i,1:34) = strjust(sprintf('%4s', name{i}), 'left');
      tidestruc.name(i,1:4) = name0(k,:);
      tidestruc.freq(i,1) = frequency0(k);
      tidestruc.tidecon(i,1:4) = [amplitude(i) 0 phase(i) 0];
  end
