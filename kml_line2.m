...
%
% input  :
% lon         --- Longitude in cell
% lat         --- Latitude in cell
% 'Model'     --- Model name
...
%
%==========================================================================
function k = kml_line2(lon, lat, fout, varargin)
...
for i = 1 : n
    lon_tmp = lon{i};
    lat_tmp = lat{i};
    for j = 1 : ceil(length(lon{i})/slice)
        i1 = (j-1)*slice + 1;
        i2 = min(j*slice, length(lon{i}));
        k.plot(lon_tmp(i1:i2), lat_tmp(i1:i2), ...
                'altitude', alt(i), ...
